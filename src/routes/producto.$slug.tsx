...
import { getProduct, PRODUCTS, PRICE, ORIGINAL_PRICE, SIZES, CONTACT, PRINT_SPEC, XXL_SURCHARGE_COP } from "@/lib/drop-data";
          ...
          <div>
            ...
            {!isInternational && (
              <p className="text-sm text-white/60 mt-1 font-mono">
                ≈ {formatAlt(price)}{" "}
                <span className="text-xs opacity-60">(TRM {formatCOP(trm)}/USD)</span>
              </p>
            )}
            ...
            {isInternational && (
              <p className="text-xs text-white/45 mt-2 leading-relaxed">
                Precios en dólares calculados con la TRM real del día ({formatCOP(trm)}/USD). El
                cobro se procesa en pesos colombianos (COP); no somos responsables por la
                conversión ni por los cargos que aplique el banco emisor de tu tarjeta.
              </p>
            )}
          </div>
